...
data_dir = '/home/pi/Desktop/PanelData/data'
path = "/home/pi/Desktop/PanelData/data/log-{}.csv".format(file_no)

MAX_BYTE_SIZE = 512  # limit byte size for csv file
MAX_FILE_NUM = 10
...
def write_data():
    ...
    data = [
    {'TIMESTAMP': '{}'.format(strftime("%Y-%m-%d %H:%M:%S")), 'POWER': POWER, 'VOLTAGE': VOLTAGE, 'CURRENT': CURRENT}
    ]
    
    field_names = ['TIMESTAMP', 'POWER', 'VOLTAGE', 'CURRENT']
    ...
    for root, dirs, files in os.walk(data_dir):
        ...
        for file in files:
            ...
            if file.endswith('.csv'):
                data_zip.write(os.path.join(root, file), os.path.relpath(os.path.join(root, file), data_dir), compress_type=zipfile.ZIP_DEFLATED)
...
def Compute():
    ...
    while status:
        ...
        if file_no is 5 and os.path.getsize(path) > MAX_BYTE_SIZE-100:
           compress_files()
           #send_log_files_to_admin()
           os.system('rm -r /home/pi/Desktop/PanelData/data')
           set_file_no()
